...
- ﻿用于描述接口或类中声明的变量。宇段（field）包括==类级变量以及实例级变量==，但是不包括方法内部、代码块内部声明的局部变量(local variables)。 
...
4. ﻿﻿启动加载器现在是在jvm内部和java类库共同协作实现的类加载器（以前是C++实现了，但为了与之前代码兼容，在获取启动类加载器的场景中仍然会返回nul1，而不会到`BootClassLoader`实例。
